...
// structure.
//
// The sum of all the entries of levelBits + logPallocChunkBytes should equal heapAddrBits.
...
var levelShift = [summaryLevels]uint{
	heapAddrBits - summaryL0Bits,                      // shift linear address by 34 to acquire index into level 0
	heapAddrBits - summaryL0Bits - 1*summaryLevelBits, // shift linear address by 31 to acquire index into level 1
	heapAddrBits - summaryL0Bits - 2*summaryLevelBits, // shift linear address by 28 to acquire index into level 2
	heapAddrBits - summaryL0Bits - 3*summaryLevelBits, // shift linear address by 25 to acquire index into level 3
	heapAddrBits - summaryL0Bits - 4*summaryLevelBits, // shift linear address by 22 to acquire index into level 4
}
...
var levelLogPages = [summaryLevels]uint{
	logPallocChunkPages + 4*summaryLevelBits, // 21, pallocSum in level zero represents at max 1<<21 go runtime pages
	logPallocChunkPages + 3*summaryLevelBits, // 18
	logPallocChunkPages + 2*summaryLevelBits, // 15
	logPallocChunkPages + 1*summaryLevelBits, // 12
...
func (p *pageAlloc) sysInit() {
	...
	for l, shift := range levelShift {
		...
		r := sysReserve(nil, b)
		if r == nil {
			panic("failed to reserve page summary memory")
		}
...
func (p *pageAlloc) sysGrow(base, limit uintptr) {
	...
	summaryRangeToSumAddrRange := func(level, sumIdxBase, sumIdxLimit int) addrRange {
		...
		return addrRange{
			base:  offAddr{uintptr(add(base, baseOffset))},
			limit: offAddr{uintptr(add(base, limitOffset))},
		}
	}
	...
	//
	// Because this function will never be asked to remap the same memory
	...
	// mapped (responsible for memory in Prepared state or Ready state) before
	// and after this new range.
	...
	for l := range p.summary {
		...
		// Prune down to what needs to be newly mapped. Some parts of it may
		// already be mapped by what inUse describes due to page alignment requirements
		// for mapping (blockAlignSummaryRange() in addrRangeToSummaryRange()).
		// prune's invariants are guaranteed by the fact that this
		// function will never be asked to remap the same memory twice (need will never
		// be equal to some addRange contained in inUse)

		// if base of the base-limit addr range needs to be appended
		// or inserted somewhere in between existing addr ranges,
		// maybe need to prune the left side of the base-limit addr range
		if inUseIndex > 0 {
			need = need.subtract(addrRangeToSumAddrRange(l, p.inUse.ranges[inUseIndex-1]))
		}

		// if base of the base-limit addr range needs to be prepended
		// or inserted somewhere in between existing addr ranges,
		// maybe need to prune the right side of the base-limit addr range
		...
		// TODO: sysUsed(unsafe.Pointer(need.base.addr()), need.size(), need.size())
